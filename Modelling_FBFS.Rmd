...
```{r fig6,echo=FALSE,fig.width=max_plots_width_in,fig.height=max_plots_height_in/3,dpi=min_plots_res,fig.cap="Effect of soil types, manure, and flood water on the available soil water content in FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}
...
p1 <- ggplot(data = ssp, aes(x=variable, y = value, fill = variable))+
  ...
  theme(
    # strip.placement = 'outside',
        ...
        axis.text = element_text(size = plot_font_size-4)
...
p1$left = grid::textGrob("Probability of available soil water", rot = 90, gp = gpar(fontsize=plot_font_size-2, face = "bold", col="black"))
p1$bottom <-  grid::textGrob("Factors of available soil water", gp = gpar(fontsize=plot_font_size-2, face = "bold", col="black"))
...
## Highliting the nodes of interest ####

highlit_some_nodes <-   grepl(" Farming_constraints_|Expected|Actual|Exploitable_yield_gap|Exploited_yield" , nodes(net))
highlit_some_nodes <- nodes(net)[highlit_some_nodes]
highlit_some_nodes <- gsub("_", " ", highlit_some_nodes)

highlit_some_FC_nodes_col <- rep("yellow3", 2) #rep("lightgoldenrodyellow", 2) # rep(adjustcolor( "yellow", alpha.f = 0.2), 2)
highlit_some_MC_nodes_col <- rep("darkgreen", 8)  # rep("forestgreen", 8) # rep(adjustcolor( "forestgreen", alpha.f = 0.2), 8)
...
graphviz_plot_net(net, shape="ellipse", layout = "dot", cex = 0.5, 
                  plot_bg = "white", # "#DFE3EE",
                  highlit_some_nodes = highlit_some_nodes,
                  highlit_some_nodes_col = highlit_some_nodes_col,
                  abbreviate = FALSE, 
                  node_col = "black", # "white", 
                  edge_col = "black", # "white", 
                  node_lwd = 0.75, edge_lwd = 1,
                  textCol = "black", split_labels_accross_lines = FALSE)
dev.off()
```

```{r fig7, echo=FALSE, out.width = '100%', fig.cap="MC model describing important variables as part of a mixed model describing the expected crop yield at different stages of crop development in FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}
...
p1$top = grid::textGrob("The yellow and grey colors are respectively values inside and outside the 90% confidence interval. ",
                        x=1, y=0, hjust=1, vjust=1, 
                        gp = gpar(fontfamily=plots_font_family,fontsize=plot_font_size-2,fontface="italic", col="grey"))

p1$left = grid::textGrob("Density", rot = 90,  
                         gp = gpar(fontfamily=plots_font_family,fontsize=plot_font_size+2, fontface = "bold", col="black"))

p1$bottom <-  grid::textGrob("Biomass yield", 
                             gp = gpar(fontfamily=plots_font_family,fontsize=plot_font_size-2, fontface = "bold", col="black"))
...
```{r fig8,echo=FALSE,fig.width=max_plots_width_in,fig.height=max_plots_height_in/2.3,dpi=min_plots_res,fig.cap="Expected biomass yield and other important yield-related metrics at initial stage of crop development in FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}

grid.draw(g)
ggsave(plot=g, device = 'png', filename = "figures/Modelling_FBFS_biomass_yield_initial_stage.png", width = max_plots_width_in, 
       height =max_plots_height_in/2.3, units = "in", dpi = min_plots_res
)
```
...
```{r fig9,echo=FALSE,fig.width=max_plots_width_in,fig.height=max_plots_height_in/2.3,dpi=min_plots_res,fig.cap="Expected biomass yield and other important yield-related metrics at late stage of crop development in FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}

grid.draw(g)
ggsave(plot=g, device = 'png', filename = "figures/Modelling_FBFS_biomass_yield_late_stage.png", width = max_plots_width_in, 
       height =max_plots_height_in/2.3, units = "in", dpi = min_plots_res
)
```
...
```{r fig10, echo=FALSE, out.width = '100%', fig.cap="Overview of causality defining the Farming constraints in FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}
...
```{r fig11,echo=FALSE,fig.width=max_plots_width_in,fig.height=max_plots_height_in/2.5,dpi=min_plots_res,fig.cap="Effect of Pest and diseases, weeds, and soil water on grain yield of rice and sorghum under FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}

p1 <- ggplot (data = ssp, aes(x, y, color = Crop_type, fill = Crop_type))+
  # geom_smooth(method = 'loess', aes(fill = Crop_type))+
  # guides(fill=FALSE)+
  geom_smooth(method = 'loess', size = 0.25, alpha = 0.2)+
...
# p1$top = grid::textGrob("Biomass Yield at Initial stage",
#                         x=0,hjust=0, gp = gpar(fontsize=16, face = "bold", col="grey"))
p1$left = grid::textGrob("Density", rot = 90, gp = gpar(fontsize=plot_font_size-2, face = "bold", col="black"))
p1$bottom <-  grid::textGrob("Grain yield", gp = gpar(fontsize=plot_font_size-2, face = "bold", col="black"))
grid.newpage()
ga = do.call("grid.arrange", p1)
gb = grid.rect(.5,.5,width=unit(1,"npc"), height=unit(1,"npc"), 
               gp=gpar(lwd=1, fill=NA, col="grey"))
