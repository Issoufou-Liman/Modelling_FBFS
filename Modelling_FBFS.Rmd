...
```{r figure_3, echo=FALSE, out.width = '100%', fig.cap="\\label{figures:figure_3}Overview of the approach used to develop the crop model for FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}
...
devtools::install_github("Issoufou-Liman/decisionSupportExtra", build = TRUE, force = TRUE, upgrade = "always", build_opts = c("--no-resave-data", "--no-manual"))
...
## Water supply adequacy partial  ####

soil_water_net <- bnlearn::model2network("[SoilType][ManureApp][SoilHoldWater|SoilType:ManureApp][EvapoTranspirat][InitWaterCont][RainAmount][FloodReachsPlot][AvailSoilWater|SoilHoldWater:EvapoTranspirat:RainAmount:InitWaterCont:FloodReachsPlot]")

abbr_node_names <- bnlearn::nodes(soil_water_net)

soil_water_net <- bnlearn::model2network("[Soil_type][Manure_application][Soil_water_holding_capacity|Soil_type:Manure_application][Evapotranspiration_at_initial_stage][Initial_soil_water_content][Rainfall_amount_at_initial_stage][Amount_of_flood_reaching_the_plot_at_initial_stage][Available_soil_water_at_initial_stage|Soil_water_holding_capacity:Evapotranspiration_at_initial_stage:Rainfall_amount_at_initial_stage:Initial_soil_water_content:Amount_of_flood_reaching_the_plot_at_initial_stage]")

legende <- bnlearn::nodes(soil_water_net)

soil_water_net <- decisionSupportExtra::extract_bn(bn = network_bn_fit, string_model = soil_water_net)

bnlearn::nodes(soil_water_net) <- abbr_node_names[c(3, 1, 2, 4, 5, 6, 8, 7)]
...
bn_legend_bg <- adjustcolor( "yellow", alpha.f = 0.15)
plots_lwd <- 0.5
min_plots_width_in <- 2.63
max_plots_width_in <- 7.5
max_plots_height_in <- 8.75
min_plots_res <- 300
bnplots_compression <- "lzw"
bnplots_family <- 'serif' # 'sans'
bnplots_font <- 1 # 2
...
png(export_fun(export = "Modelling_FBFS_Avail_soil_water"),
     res = min_plots_res,
     units = 'in',
     # compression = bnplots_compression,
     width = max_plots_width_in, 
     height = max_plots_height_in/2.75,
     pointsize = 10.5)

par(font=bnplots_font, family = bnplots_family, lwd=plots_lwd)

graphviz_chart_bn (x = soil_water_net, type = "barprob", layout = "dot", draw.levels = TRUE,abbreviate=FALSE,
...
legend('topleft', legend = legende, text.width = 0.6*strwidth(legende[which.max(nchar(legende))]), 
       cex=0.6, ncol = 2,
       bty="o", box.lwd=1, box.col='lightyellow', xjust=1, yjust=1, bg=bn_legend_bg,
       title = 'Legend', title.col = 'lightgreen', inset=c(0.04, 0.08))

dev.off()
```

```{r figure_5, echo=FALSE, out.width = '100%', fig.cap="\\label{figures:figure_5}Illustration of the causality defining the factor of soil water content in FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}
...
evidence <- c("Manure_application", "Soil_type", "Amount_of_flood_reaching_the_plot_at_initial_stage")
evidence <- sapply(evidence, function (i){
  dimnames(network_bn_fit[[i]][['prob']])[[1]]
})

evidence <- expand.grid(evidence, stringsAsFactors = FALSE)
evidence <- evidence[c('Soil_type', 'Manure_application', 'Amount_of_flood_reaching_the_plot_at_initial_stage')]

bloks = 1:10
evidence <- lapply(bloks, function(i){
  cbind(block = rep(i, nrow(evidence)), evidence)
})
evidence <- do.call(rbind, evidence)
evidence <- evidence[order(evidence$Amount_of_flood_reaching_the_plot_at_initial_stage),]
...
evidence_list <- make_evidence(sapply(evidence[, -1], as.character))
...
evidence_list <- sapply(evidence_list, format_bnlearn_evidence, simplify = FALSE, USE.NAMES = TRUE)


ssp = sapply(evidence_list, function(i) {
  out = parse(text=paste0('bnlearn::cpdist', '(', 'network_bn_fit', ',', "'", 'Available_soil_water_at_initial_stage',"'" , ',', i,')'))
  out = eval(out)
  out = table(out)
...
ssp = data.frame(evidence, do.call(rbind, ssp), stringsAsFactors = FALSE)
ssp$query <- lapply(1:nrow(ssp), function(i){
  paste0((ssp[i, 2:4]), collapse = "\n")
})

ssp$query <- factor(ssp$query, levels = unique(ssp$query))
ssp  <- reshape::melt(ssp, id.vars = names(ssp)[c(1:4, 8)])
library(ggplot2)
bnplots_family <- 'serif'
bnplots_font <- 12
my_theme <- theme_bw(base_size = bnplots_font+4, base_family = bnplots_family) +
...
```{r figure_6,echo=FALSE,fig.width=14.2,fig.height=7.14,dpi=min_plots_res,fig.cap="\\label{figures:figure_6}Effect of the states of soil type manure application and flood water on the available soil water content at initial stage of crop development in Kisumu (Kenya) and Tigray (Ethiopia)."}

ggplot(data = ssp, aes(x=variable, y = value, fill = variable))+
  geom_boxplot()+
  ...
  labs(x = "Factors of available soil water", y = "Probability of available soil water")+
  my_theme +
  # switch the facet strip label to outside 
  # remove background color
  theme(strip.placement = 'outside',
        axis.text.x = element_blank(),
         axis.title = element_text(size = bnplots_font+8),
        legend.text = element_text(size = bnplots_font+8),
        legend.position = 'top',
