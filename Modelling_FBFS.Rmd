...
model_function <- function(x){
  ...
  exploitable_yield_gap_loss_due_to_constraints_at_development_stage <- ifelse(exploitable_yield_gap_loss_due_to_constraints_at_development_stage <= exploitable_yield_gap_at_development_stage, exploitable_yield_gap_loss_due_to_constraints_at_development_stage, exploitable_yield_gap_at_development_stage) # because exploitable_yield_gap_loss_due_to_constraints_at_development_stage cannot exceed the exploitable_yield_gap_at_development_stage itself.
    ##############################################################################################
  actual_exploited_yield_gap_at_development_stage <- exploitable_yield_gap_at_development_stage - exploitable_yield_gap_loss_due_to_constraints_at_development_stage
  ...
  if (i==2){
    ...
    p_i
      ...
      if (i == 1){
        p_i <- p_i +
          scale_x_continuous(breaks = x_breaks[[i]], limits = x_limits[[i]], labels = x_scaleFUN)+
          scale_y_continuous(breaks = c(0, 0.5, 1), labels = y_scaleFUN) # making some space for this one as labels overlap.
      } else {
        p_i <- p_i +
          scale_x_continuous(breaks = x_breaks[[i]], limits = x_limits[[i]], labels = x_scaleFUN)
      }
...
p1$top = grid::textGrob("The green and red colors, respectively, are values inside and outside the 90% confidence interval. Values on x-axis are in t/ha.",
...
p1 <- ggplot (data = ssp[ssp$color=='green', ], aes(x, fill = Crop_type, color = Crop_type))+
  ...
  theme(
    ...
    strip.text = element_text(size = plot_font_size-2.5, face = 'bold'),
    strip.text.x = element_text(margin = margin(t=3, b = 3)),
    axis.text = element_text(size = plot_font_size),
  ...
  function(d){
    annotation_custom2(
      grob = ggplotGrob(ggplot(data.frame(x=d$Crop_type, y=d$x))+
                          geom_boxplot(aes(x=x, y=y, fill=x, color=x), alpha=0.1, lwd=0.0005, outlier.size = 0.05)+
                          my_theme+
                          theme(
                            ...
                            axis.text.x = element_text(size=plot_font_size, margin = margin(c(0.5,0,0,0), unit = 'mm')),
...
p1 <- list(p1)
p1$ncol=1
p1$top = grid::textGrob("Values on the x-axis of the subplots (boxplots) are the ranges of simulated grain yield in tons per hectare.Those on the y-axis of the main plots are the scaled density 
(along a gradient of farming constraint).Those on the x-axis of the main plots are values in the boxplots truncated in the near zero probability (scaled density). 
FC stands for farming constraint and the indices i, ii, iii, iiii, respectively, represent crop at initial, developement, mid and late stages.",
...
ggsave(plot=g, device='png', filename = "figures/Modelling_FBFS_grain_yield.png", width = max_plots_width_in, 
       height=max_plots_height_in, units="in", dpi=min_plots_res
)
```
