...
my_theme <- theme_bw(base_size = plot_font_size + 4, base_family = plots_font_family) +
...
```{r case study 1 methods, include = FALSE, echo = FALSE, cache = FALSE}
...
```{r case study 1 results, include = FALSE, echo = FALSE, cache = FALSE}
...
ggplot(data = ssp, aes(x=variable, y = value, fill = variable))+
  ...
  theme(strip.placement = 'outside',
        axis.text.x = element_blank(),
         axis.title = element_text(size = plot_font_size+8),
        legend.text = element_text(size = plot_font_size+8),
        legend.position = 'top',
...
```{r case study 3 methods, include = FALSE, echo = FALSE, cache = FALSE}
...
```{r figure_7, echo=FALSE, out.width = '100%', fig.cap="\\label{figures:figure_7}Overview of causality defining the Farming constraints in FBFS in Kisumu (Kenya) and Tigray (Ethiopia)."}
...
```{r case study 3 results, include = FALSE, echo = FALSE, cache = FALSE}
## constructing the evidence list ####
...
# ssp <- readRDS("output_files/190512092837_model_predictions_mid_stage_constraints_based_query.rds")

ssp = sapply(X = 1:length(ssp), FUN = function(i){
  out <- ggplot_mc_dens(ssp[[i]])$data
  # out <- out[out$x >= 0, ]
...
head(ssp)
# ssp <- reshape2::melt(ssp, id.vars = c("x", "y", "color", "color_equiv", "quant", "variable", "value", 
#                                        "L5", "L4", "L3", "L2", "L1", "trt", "Crop_type"))
# head(ssp)
...
```{r figure_8,echo=FALSE,fig.width=14.2,fig.height=7.14,dpi=min_plots_res,fig.cap="\\label{figures:figure_6}Effect of the states of soil type manure application and flood water on the available soil water content at initial stage of crop development in Kisumu (Kenya) and Tigray (Ethiopia)."}

ggplot (data = ssp, aes(x, y, color = Crop_type))+
  ...
  # remove background color
  theme(
        axis.title = element_text(size = plot_font_size + 6),
